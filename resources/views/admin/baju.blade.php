    ...
    <div class="container">
            ...
            @foreach ($baju as $data)
                <div class="col-3">
                    <div class="card" style="width: 15rem; height:23rem;">
                        <img src="{{ asset('storage/' . $data->gambar_baju) }}" class="card-img-top rounded-top" alt="..."
                            height="200" style="object-fit:cover">
                        <div class="card-body">
                            <a href="#">
                                <h5 class="card-title">{{ Str::limit($data->name_baju, 30) }}</h5>
                            </a>
                            <p class="card-text">{!! Str::limit($data->deskripsi_baju, 70) !!}</p>
                            <div class="align-items-start">
                                <button type="button" class="btn btn-primary btn-sm btn-show"
                                    data-name="{{ $data->name_baju }}"
                                    data-img="{{ asset('storage/' . $data->gambar_baju) }}"
                                    data-deskripsi="{!! $data->deskripsi_baju !!}" data-id="{{ $data->id }}">Lihat</button>
                                    <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#exampleModal{{ $data->id }}">
                                        Edit
                                    </button>
                                <a href="#" data-id="{{ $data->id }}" data-nama="{{ $data->name_baju }}"
        ...
        <div class="modal fade" id="modal-lg">
            <div class="modal-dialog modal-lg">
                <form action="/insertbaju" method="POST" enctype="multipart/form-data" class="row g-3">
                    @method('POST')
                    @csrf
                        ...
                        <div class="modal-body">
                            <div class="col-12 mb-2">
                                ...
                                <div class="input-group mb-3">
                                    <input type="file" class="form-control @error('gambar_baju') is-invalid @enderror"
                                        name="gambar_baju" id="gambar_baju">
                                    @error('gambar_baju')
                            ...
                            <div class="col-12 mb-2">
                                ...
                                <input type="text" class="form-control @error('name_baju') is-invalid @enderror"
                                    name="name_baju" id="name_baju">
                                @error('name_baju')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12 mb-2">
                                ...
                                <textarea class="form-control @error('deskripsi_baju') is-invalid @enderror" id="editor" name="deskripsi_baju"
                                    cols="30" rows="10" placeholder="Tuliskan deskripsi baju disini"></textarea>
                                @error('deskripsi_baju')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form method="POST" action="/update-baju/{{$data->id}}" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="col-12 mb-2">
                            <label class="fw-bold">Masukkan Gambar</label>
                            <div class="input-group mb-3">
                                <img src="{{ asset('storage/' . $data->gambar_baju) }}" width="100%" alt="">
                                <input type="file" class="form-control @error('gambar_baju') is-invalid @enderror"
                                    name="gambar_baju" id="gambar_baju">
                                @error('gambar_baju')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-12 mb-2">
                            <label for="judul" class="form-label fw-bold">Nama Produk</label>
                            <input type="text" class="form-control @error('name_baju') is-invalid @enderror"
                                name="name_baju" id="name_baju" value="{{ $data->name_baju }}">
                            @error('name_baju')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-12 mb-2">
                            <label for="subjudul" class="form-label fw-bold">Kategori</label>
                            <select type="text" class="form-control @error('kategori_id') is-invalid @enderror kategori"
                                aria-label="Default select example" name="kategori_id" id="kategori_id">
                                <option selected>Pilih Kategori</option>
                                @foreach ($kategori as $option)
                                        <option value="{{ $option->id }}" {{ $option->id == $data->kategori_id ? 'selected' : '' }}>{{ $option->name_kategori }}</option>
                                    @endforeach
                            </select>
                            @error('kategori_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-12 mb-2">
                            <label for="isi" class="form-label fw-bold">Deskripsi Produk</label>
                            <textarea class="form-control @error('deskripsi_baju') is-invalid @enderror" id="editor"
                                name="deskripsi_baju" cols="30" rows="10" placeholder="Tuliskan deskripsi baju disini">{!! $data->deskripsi_baju !!}</textarea>
                            @error('deskripsi_baju')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
    ...
    <script>
        $('.delete').click(function() {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        window.location = "/deletebaju/" + bajuid + ""
