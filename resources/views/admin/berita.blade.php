    ...
    <div class="container">
            ...
            @foreach ($berita as $data)
                <div class="col-3">
                    <div class="card" style="width: 15rem; height:25rem;">
                        ...
                        <div class="card-body">
                            <a href="#">
                                <h5 class="card-title">{{ Str::limit($data->judul_berita, 30) }}</h5>
                            </a>
                            <p class="card-text">{!! Str::limit($data->isi, 70) !!}</p>
                            <div class="align-items-start">
                                <button type="button" class="btn btn-primary btn-sm btn-show"
                                    data-name="{{ $data->judul_berita }}" data-img="{{ asset('storage/' . $data->image) }}"
                                ...
                                <a href="#" data-id="{{ $data->id }}" data-nama="{{ $data->judul_berita }}"
        ...
        <div class="modal fade" id="modal-lg">
            <div class="modal-dialog modal-lg">
                <form action="/insertberita" method="POST" enctype="multipart/form-data" class="row g-3">
                    @method('POST')
                    @csrf
                        ...
                        <div class="modal-body">
                            ...
                            <div class="col-12 mb-2">
                                ...
                                <textarea class="form-control @error('isi') is-invalid @enderror" id="editor" name="isi" cols="30"
                                    rows="10" placeholder="Tuliskan deskripsi baju disini"></textarea>
                                @error('isi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
    ...
    <div class="modal fade" id="exampleModal{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="/update-berita/{{$data->id}}" enctype="multipart/form-data" method="POST">
                    <div class="modal-body">
                        ...
                        <div class="col-12 mb-2">
                            ...
                            <input type="text"
                                class="form-control @error('judul_berita') is-invalid @enderror judul_berita"
                                name="judul_berita" id="judul_berita" value="{{$data->judul_berita}}">
                            @error('judul_berita')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-12 mb-2">
                            ...
                            <textarea class="form-control @error('isi') is-invalid @enderror" id="isi" name="isi" cols="30"
                                rows="10" placeholder="Tuliskan deskripsi baju disini">{!!$data->isi!!}</textarea>
                            @error('isi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
    ...
    <script>
        $('.delete').click(function() {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        window.location = "/deleteberita/" + bajuid + ""
